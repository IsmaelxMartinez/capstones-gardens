...
const Customize = () => {
  ...
  const [response, setResponse] = useState(null); // Change to null for JSON parsing
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Clean the response to remove code block delimiters
      const cleanedResponse = res.data.response
        .replace(/```json|```/g, "")
        .trim();

      setResponse(JSON.parse(cleanedResponse)); // Parse the cleaned JSON response
    } catch (error) {
